...
class YoloPoseWrapper(YoloBaseWrapper):
    def detect(self, img: np.ndarray, conf: float = 0.4):
        ...
        indices = cv2.dnn.NMSBoxes(
            preds[:, :4].tolist(), scores.tolist(), conf, 0.5)
        if len(indices) == 0:
            return []
        ...
        for i in indices.flatten():
            ...
            kpts = [{"x": int(p[0]*sx), "y": int(p[1]*sy),
                     "conf": float(p[2])} for p in pk]
...
class HitDetectionServiceOptimized:
    def __init__(self,
                 ...
                 # usar modelo pequeño por compatibilidad OpenCV
                 model_name_depth="midas_v21_small",
                 yolo_size=640,
        ...
        # Compat con código legacy que esperaba .det
        self.det = self.det_loader
        ...
        # Compat con código legacy que esperaba .pose
        self.pose = self.pose_loader
    ...
    def compare_face_base64(self, face_a_b64: str, face_b_b64: str):
        ...
        try:
            ...
            import numpy as np
            import cv2
            payload = face_a_b64.split(
                ",")[1] if "," in face_a_b64 else face_a_b64
            img_a = cv2.imdecode(np.frombuffer(
                base64.b64decode(payload), np.uint8), cv2.IMREAD_COLOR)
            payload = face_b_b64.split(
                ",")[1] if "," in face_b_b64 else face_b_b64
        ...
        try:
            ...
            if not fa or not fb:
                return None
            sim = self.face_service.compare_embeddings(
                fa['embedding'], fb['embedding'])
    ...
    def process_frame(self, frame):
        ...
        res_det = self.det_loader.model(
            frame, imgsz=self.yolo_size, verbose=False, conf=0.25)[0]
        res_pose = self.pose_loader.model(
            frame, imgsz=self.yolo_size, verbose=False, conf=0.25)[0]
        ...
        if self.depth_loader.backend == 'opencv':
            ...
            blob = cv2.dnn.blobFromImage(
                ...
                # Media ImageNet (aprox) si el modelo lo requiere, o 0 si es standard
